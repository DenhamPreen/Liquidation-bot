...
export class Metrics {
  ...
  constructor(options: {
    ...
    if (
      typeof process.env.PROM_AUTH === "undefined" ||
      process.env.PROM_AUTH.trim().length === 0
    ) {
  ...
  increment(name: string, values?: string[]): void {
    const counter =
      this.counters.get(name) || new Counter(this.createConf(name));
  ...
  private createConf(name: string): {
    help: string;
    labelNames: string[];
    name: string;
  } {
