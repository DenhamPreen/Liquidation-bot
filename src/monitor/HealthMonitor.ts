...
export class HealthMonitor extends AbstractMonitor<boolean> {
  ...
  private async checkPairs() {
    ...
    // console.log("pairs");
    // console.log(pairsWithPositions);
    ...
    pairWithPositionsTotal
      ...
      .forEach((pair) => {
        addBreadcrumb(
          "pair",
          pair.address,
          `${pair.short ? "SHORT" : "LONG "} ${pair.path} totalSupply: ${bn(
            pair.totalSupply
          ).human(
        ...
        this.context.metrics.update("pair_total_supply", {
          [pair.path]: Metrics.format(bn(pair.totalSupply)),
        });
    ...
    const notSyncedPairs = pairWithPositionsTotal
      .map(({ queryBottom, totalSupply, positionTotal }) => {
    ...
    this.context.metrics.update(
      "pairs_not_synced_count",
      notSyncedPairs.length
    );
  ...
  private async checkPositions() {
    ...
    positions.forEach((pos) => {
      this.context.metrics.update("pos_health", {
        [[pos.pair, pos.trader].join("_").toLowerCase()]: pos.health.eq(0)
          ? -1
          : Metrics.format(pos.health) / 10_000_000,
      });
      this.context.metrics.update("positions_profit_percent", {
        [[pos.pair, pos.trader].join("_").toLowerCase()]: Metrics.format(
          Position.profitPercent(pos),
          2
        ),
      });
    });
    ...
    this.context.metrics.update("positions_health", {
      expired: expired_positions.length,
      not_updated: not_updated_positions.length,
    });
