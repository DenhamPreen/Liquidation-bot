...
export function init(
  ...
  sentryInit({
    ...
    beforeBreadcrumb: (crumb, _) => {
      if (crumb.category === "console" && crumb.message?.charAt(0) === "[")
        return null; // Remove logs from addBreadcrumb
        ...
        event.breadcrumbs = (event.breadcrumbs || [])
          .filter(
            (crumb) =>
              ...
              (crumb.data?.category === event.tags?.category &&
                crumb.data?.tag === event.tags?.tag)
          ) // Remove different breadcrumbs
          .map((crumb) => {
...
export function addMessage(
  ...
  captureMessage(message, {
    tags: { category, tag: tag.toLowerCase() },
    extra,
  });
}
